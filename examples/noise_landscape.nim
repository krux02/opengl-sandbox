...
let indices = elementArrayBuffer(heightmap.indices)

let indicesLen = indices.len

var running = true

var projMat = perspective(45'f32, windowsize.x / windowsize.y, 0.1, 100.0)
...
while running:
  ...
  camera.moveAbolute(vec3f(64,64,0))

  let time = gameTimer.time.float32

  camera.turnRelativeX(radians(15.0f))
...
// Mix final noise value
  ...
  return 42.0 * dot( m*m, vec4( dot(p0,x0), dot(p1,x1), 
    ...
    vertexMain:
      """
      gl_Position = mvp * (pos + vec4(0,0,snoise(vec3(pos.xy*0.123,time))*5,0));
      """
    fragmentMain:
      """
      color = vec4(1);
